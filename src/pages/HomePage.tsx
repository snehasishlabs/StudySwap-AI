...
export const HomePage: React.FC = () => {
  ...
  const handleQuickExamSearch = (examId: ExamCategory) => {
  ...
  return (
    <div className="space-y-16 sm:space-y-24 pb-20">
      {/* Authentic StudySwap Hero Section */}
      <section className="relative overflow-hidden pt-10 sm:pt-16 pb-14 sm:pb-20 bg-gradient-to-b from-sky-100/60 via-blue-50/40 to-white text-slate-900 border-b border-blue-100/80">
        {/* Ambient atmospheric backdrop */}
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute -top-32 -left-32 w-96 h-96 rounded-full bg-blue-300/20 blur-[100px]" />
          <div className="absolute top-1/3 -right-32 w-96 h-96 rounded-full bg-sky-200/25 blur-[100px]" />
          <div className="absolute -bottom-20 left-1/2 -translate-x-1/2 w-3/4 h-32 bg-blue-200/20 blur-[80px]" />
          <div 
            className="absolute inset-0 opacity-[0.035]" 
            style={{ 
              backgroundImage: `radial-gradient(circle at 1px 1px, rgba(30,58,138,0.7) 1px, transparent 0)`,
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10">
          ...
          <div className="flex items-center justify-center gap-2 mb-6">
            <div className="inline-flex items-center gap-2 bg-blue-50 text-blue-700 text-xs font-semibold px-4 py-1.5 rounded-full border border-blue-200/80 shadow-xs">
              <Sparkles className="w-3.5 h-3.5 text-blue-600" />
              <span>India's Premier AI-Assisted Student Study Marketplace</span>
            </div>
          </div>
          ...
          <div className="text-center max-w-4xl mx-auto space-y-4">
            <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black tracking-tight leading-[1.15] text-slate-900">
              ...
              <span className="bg-gradient-to-r from-blue-600 via-sky-600 to-indigo-600 bg-clip-text text-transparent">
                Verified Books & Notes
              </span>
            </h1>

            <p className="text-base sm:text-lg text-slate-600 max-w-2xl mx-auto leading-relaxed">
              Save up to 75% on standard competitive exam books, topper handwritten summaries, and study gear. Connect directly with peer aspirants for safe campus handoffs.
            </p>

            {/* Instant Search Bar */}
            <form onSubmit={handleHeroSearch} className="pt-4 max-w-2xl mx-auto">
              <div className="relative flex items-center bg-white rounded-2xl p-1.5 shadow-xl shadow-blue-900/5 border border-blue-200/70 focus-within:ring-2 focus-within:ring-blue-500 transition">
                ...
                <input
                  ...
                  placeholder="Search books, topper notes, syllabus, author (e.g. Laxmikanth, Aggarwal, NCERT)..."
                  className="w-full bg-transparent px-3 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none"
                />
                <button
                  type="submit"
                  id="hero-search-submit-btn"
                  className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold text-sm px-6 py-3 rounded-xl shadow-md shadow-blue-600/20 transition cursor-pointer shrink-0 flex items-center gap-1.5"
                >
                  <span>Search</span>
            ...
            <div className="pt-2 flex flex-wrap items-center justify-center gap-2 text-xs">
              <span className="text-slate-500 font-semibold mr-1">Popular Exams:</span>
              {['UPSC', 'SSC', 'Banking', 'GATE', 'JEE', 'NEET', 'WBPSC', 'CAT', 'Railways'].map((exam) => (
                <button
                  key={exam}
                  type="button"
                  onClick={() => handleQuickExamSearch(exam as ExamCategory)}
                  className="px-2.5 py-1 rounded-lg bg-white/90 hover:bg-blue-50 text-slate-700 hover:text-blue-700 border border-slate-200/90 hover:border-blue-300 text-xs font-medium shadow-2xs transition cursor-pointer"
                >
                  {exam}
                </button>
              ))}
            ...
            {/* Core Action CTAs: Explore Marketplace, Buyer Demo, Seller Demo */}
            <div className="pt-6 flex flex-wrap items-center justify-center gap-3.5">
              <button
                type="button"
                id="hero-explore-marketplace-btn"
                onClick={() => navigateTo('marketplace')}
                className="bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm px-6 py-3.5 rounded-xl shadow-lg shadow-blue-600/25 transition flex items-center gap-2 cursor-pointer"
              >
              ...
              <button
                ...
                className="bg-white hover:bg-slate-50 text-slate-800 font-semibold text-sm px-5 py-3.5 rounded-xl border border-slate-300/90 hover:border-slate-400 shadow-xs transition flex items-center gap-2 cursor-pointer"
              >
                <Users className="w-4 h-4 text-blue-600" />
                <span>Buyer Demo (Rahul)</span>
              </button>

              <button
                type="button"
                id="hero-seller-demo-btn"
                onClick={() => switchToSellerDemo('add-listing')}
                className="bg-purple-50 hover:bg-purple-100/80 text-purple-700 font-semibold text-sm px-5 py-3.5 rounded-xl border border-purple-200 hover:border-purple-300 shadow-xs transition flex items-center gap-2 cursor-pointer"
              >
                <PlusCircle className="w-4 h-4 text-purple-600" />
                <span>Seller Demo (Priya)</span>
              </button>
            </div>
          </div>

          {/* Trust Highlights Grid */}
          <div className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto pt-8 border-t border-blue-200/60">
            <div className="text-center p-3 rounded-2xl bg-white/80 backdrop-blur-xs border border-blue-100 shadow-xs">
              <div className="text-xl sm:text-2xl font-black text-blue-600">₹24,50,000+</div>
              <div className="text-[11px] text-slate-500 uppercase tracking-wider font-semibold mt-0.5">Student Savings</div>
            </div>
            <div className="text-center p-3 rounded-2xl bg-white/80 backdrop-blur-xs border border-blue-100 shadow-xs">
              <div className="text-xl sm:text-2xl font-black text-emerald-600">10,000+</div>
              <div className="text-[11px] text-slate-500 uppercase tracking-wider font-semibold mt-0.5">Books Recycled</div>
            </div>
            <div className="text-center p-3 rounded-2xl bg-white/80 backdrop-blur-xs border border-blue-100 shadow-xs">
              <div className="text-xl sm:text-2xl font-black text-purple-600">100% Peer</div>
              <div className="text-[11px] text-slate-500 uppercase tracking-wider font-semibold mt-0.5">Campus Meetups</div>
            </div>
            <div className="text-center p-3 rounded-2xl bg-white/80 backdrop-blur-xs border border-blue-100 shadow-xs">
              <div className="text-xl sm:text-2xl font-black text-amber-600">Gemini AI</div>
              <div className="text-[11px] text-slate-500 uppercase tracking-wider font-semibold mt-0.5">Verified Quality</div>
            </div>
          </div>
        </div>
      </section>

      {/* 1. Target Exam Categories Section */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-9 gap-3">
          {EXAM_CATEGORIES.map((exam) => (
            <button
              key={exam.id}
              onClick={() => handleQuickExamSearch(exam.id as ExamCategory)}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Top-rated standard textbooks and AI-certified topper handwritten notes available for immediate pickup.
            </p>
          </div>
      ...
      {/* 3. Study Bundle Generator Section */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <StudyBundlesSection />
      </section>
      ...
      {/* 6. Sustainability Impact Counter */}
      <SustainabilityImpactSection />

      {/* 7. Student Savings Calculator Section */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 my-16">
        <StudentSavingsCalculator />
      </section>

      {/* 8. Aspirant Community Testimonials */}
      ...
      {/* 9. Final Marketplace Call-To-Action */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
